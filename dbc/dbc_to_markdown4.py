...
def generate_index_table(db):
    ...
    for msg in db.messages:
        ...
        table += f"| [{msg.name}](#{safe_anchor(msg.name)}) | {id_str} | {msg.length} | {comment} |\n"
    return table
...
def generate_value_map_table(msg):
    """
    For each signal that defines choices, list its value map.
    """
    rows = []
    for signal in msg.signals:
...
def generate_message_section(msg):
    section = f"**Message:** {msg.name}\n\n"
    # Table 1: Message ID and DLC
    id_table = "| ID | DLC |\n| -- | --- |\n"
    id_table += f"| {msg.frame_id} (0x{msg.frame_id:x}) | {msg.length} |\n\n"
    section += id_table

    # Comment, if any, between the first two tables
    if msg.comment:
...
def generate_markdown(db):
    ...
    # For each message, output the three tables in sequence.
    for msg in db.messages:
